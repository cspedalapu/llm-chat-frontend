...
export function Library({ documents, conversations, onUpload, onDelete, onOpen, showDocuments = true, showSaved = true }: {
  ...
  return <section className="feature-page"><div className="page-heading"><div><p className="eyebrow">Keep useful context</p><h1>Library</h1></div>{showDocuments && <button disabled={busy} className="workspace-modal-primary" onClick={() => input.current?.click()}>{busy ? "Uploading…" : "Upload documents"}</button>}</div>
    {showDocuments && <><p className="muted">Text PDFs, Word, Excel, Markdown, text, CSV, JSON and code files. Up to 10 MB and 500,000 extracted characters per file. Scanned PDFs need OCR first.</p>
    <input className="visually-hidden" ref={input} type="file" multiple accept=".pdf,.docx,.xlsx,.txt,.md,.csv,.json,.py,.js,.ts,.log" onChange={async e => { const files = Array.from(e.target.files || []); e.target.value = ""; setBusy(true); try { await onUpload(files); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }} />
