...
export function Library({ documents, conversations, onUpload, onDelete, onOpen }: {
  documents: DocumentRecord[]; conversations: Conversation[]; onUpload: (files: File[]) => Promise<void>;
  onDelete: (id: string) => void; onOpen: (id: string) => void;
}) {
  ...
  return <section className="feature-page"><div className="page-heading"><div><p className="eyebrow">Keep useful context</p><h1>Library</h1></div><button disabled={busy} className="workspace-modal-primary" onClick={() => input.current?.click()}>{busy ? "Uploading…" : "Upload documents"}</button></div>
    <p className="muted">Text PDFs, Markdown, text, CSV, JSON and code files. Up to 10 MB and 500,000 extracted characters per file. Scanned PDFs need OCR first.</p>
    ...
    <h2>Saved answers</h2>{!saved.length && <div className="empty-card">Use “Save answer” under an assistant response to keep it here.</div>}
