...
def build_request(provider, messages):
    ...
    if kind == "anthropic":
        ...
        return base + "/messages", headers, {
            "model": model, "messages": turns, "system": system,
            "max_tokens": limit, "stream": True,
        }
    if kind == "gemini":
        headers["x-goog-api-key"] = key
        return base + "/models/" + quote(model.removeprefix("models/"), safe="") + \
            ":streamGenerateContent?alt=sse", headers, {
    ...
    body = {"model": model, "messages": [
        {**m, "role": provider.get("system_role", "system")}
        if m["role"] == "system" else m for m in messages
    ], "stream": True, provider.get("token_parameter", "max_tokens"): limit}
...
def normalize_chunk(kind, data):
    if data.get("error") or data.get("type") == "error":
        raise ProviderError("Provider reported an error during generation. Check its service status.")
    if kind == "openai":
        if data.get("model"):
        ...
        if data.get("usage"):
            yield {"usage": {"input": data["usage"].get("prompt_tokens", 0),
        ...
        if usage:
            yield {"usage": {k: usage[v] for k, v in (
        ...
        if data.get("usageMetadata"):
            ...
            yield {"usage": {"input": usage.get("promptTokenCount", 0),
        ...
        if data.get("done"):
            yield {"done": True, "finish": data.get("done_reason", "stop"),
        ...
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=False) as client:
            async with client.stream("POST", url, headers=headers, json=body) as response:
                if response.status_code >= 300:
                    ...
                    hints = {401: "Check the API key.", 403: "Check model access and permissions.",
                ...
                async for line in response.aiter_lines():
                    ...
                    for event in normalize_chunk(provider["kind"], json.loads(payload)):
                        ...
                        yield event
                if not finished:
                    raise ProviderError("Provider stream ended unexpectedly. Partial output was saved.")
    except httpx.TimeoutException as exc:
        raise ProviderError("Provider timed out. Check the connection and retry.") from exc
    except httpx.HTTPError as exc:
        raise ProviderError("Could not connect to the provider. Check its base URL and service.") from exc
