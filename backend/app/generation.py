...
def ensure_idle(con, conversation_id):
    if con.execute(
        "SELECT 1 FROM generations WHERE conversation_id=? AND status='streaming'",
        (conversation_id,),
    ).fetchone():
...
def usage():
    with store.db() as con:
        rows = con.execute(
            "SELECT body,status,created_at FROM generations ORDER BY created_at DESC"
        ).fetchall()
    items = [
        {**json.loads(r["body"]), "status": r["status"], "createdAt": r["created_at"]} for r in rows
    ]
    return {
        "requests": len(items),
        "today": sum(i["createdAt"][:10] == store.now()[:10] for i in items),
...
def save_generation(conversation_id, message_id, request_id, text, state, metadata):
    with store.db() as con:
        ...
        con.execute(
            "UPDATE generations SET status=?,body=? WHERE id=?",
            (
                state,
                json.dumps(
                    {
...
async def run_generation(
    provider, messages, conversation_id, message_id, request_id, metadata, queue
):
    text = ""
    state = "ready"
    ...
    try:
        async with asyncio.timeout(300):
            async for event in providers.stream(provider, messages):
                if event.get("text"):
                    ...
                    if len(text) > 500000:
                        raise providers.ProviderError(
                            "Response exceeded the local output size limit."
                        )
                ...
                if time.monotonic() - last_save > 1:
                    save_generation(
                        conversation_id, message_id, request_id, text, "streaming", metadata
                    )
        ...
        if (
            provider.get("input_price") is not None
            and provider.get("output_price") is not None
            and "input" in usage
            and "output" in usage
        ):
            metadata["estimated_cost"] = (
                usage["input"] * provider["input_price"]
                + usage["output"] * provider["output_price"]
            ) / 1000000
        ...
        await queue.put(
            ("done", {"state": state, "text": text, "result": metadata, "message_id": message_id})
        )
...
async def generate(conversation_id: str, body: GenerateInput):
    ...
    with store.db() as con:
        ...
        if old:
            ...
            if old["status"] == "streaming":
                raise HTTPException(
                    409, "This request is already running. Refresh the conversation."
                )
        ...
        count = con.execute(
            "SELECT count(*) FROM generations WHERE created_at>=?", (store.now()[:10],)
        ).fetchone()[0]
        ...
        allowed = []
        for message in item["messages"]:
            for document_id in message.get("documentIds", []):
                doc = store.get(con, "document", document_id)
                if doc and doc.get("projectId") in (None, item.get("projectId")):
    ...
    # Enter the worker's try/finally before a stop request can cancel it.
